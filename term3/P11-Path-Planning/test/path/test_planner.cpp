...
TEST_CASE("Path planner should adjust speed to vehicle ahead", "[path]") {
  ...
  const vector<vector<double>> otherVehicles = {
    ...
    {0, x + speed * 2.5, y, speed * 0.9, 0.0, s + speed * 2.5, d},

    /* Block the other lanes with even slower vehicles. */
    {1, x + speed * 2.5, y - constants.laneWidth, speed * 0.89, 0.0, s + speed * 2.5, d + constants.laneWidth},
    {2, x + speed * 2.5, y + constants.laneWidth, speed * 0.89, 0.0, s + speed * 2.5, d - constants.laneWidth},
  };
...
TEST_CASE("Path planner should adjust speed below vehicle ahead", "[path]") {
  ...
  const vector<vector<double>> otherVehicles = {
    // Vehicle in same lane keeping highest speed.
    {0, x + speed * 2.0, y, speed * 0.9, 0.0, s + speed * 2.0, d},

    /* Block the other lanes with even slower vehicles. */
    {1, x + speed * 2.0, y - constants.laneWidth, speed * 0.89, 0.0, s + speed * 2.0, d + constants.laneWidth},
    {2, x + speed * 2.0, y + constants.laneWidth, speed * 0.89, 0.0, s + speed * 2.0, d - constants.laneWidth},
  };
  const VehicleData vehicleData(x, y, s, d, yaw, speed, otherVehicles);
...
TEST_CASE("Path planner should switch lane left", "[path]") {
  ...
  const vector<vector<double>> otherVehicles = {
    ...
    {0, x + speed * 3.0, y, speed * 0.9, 0.0, s + speed * 3.0, d},
    // Vehicle ahead in right lane going slow.
    {1, x + speed * 3.0, y - constants.laneWidth, speed * 0.9, 0.0, s + speed * 3.0, d + constants.laneWidth},
  };
  const VehicleData vehicleData(x, y, s, d, yaw, speed, otherVehicles);
  const Path::Trajectory previousPath;
  Path::Trajectory trajectory = planner.CalcNext(vehicleData, previousPath, s, d);
